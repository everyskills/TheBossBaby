...
class Results(QWidget):
    ...
    def search_app(self):
        for k, v in apps.items():
            if ((self.query and k) and 
                ...
                self.ui.list_widget.count() <= self.parent.settings("max_results", 12)):
    ...
    def set_apps(self):
        user_path = glob(os.path.expanduser(f"{self.parent.settings('user_apps')}/*.desktop"))
        user_path.extend(glob(f"{self.parent.settings('sys_apps')}/*.desktop"))
