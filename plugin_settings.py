...
class PluginSettings(QWidget):
    ...
    def check_download(self):
        ...
        if os.path.exists(url) and url.endswith(".zip"):
            ...
            # shutil.copytree(url, base_dir + down_path)
            shutil.copytree(url, base_dir + down_path + url.split("/")[-1])
    ...
    def set_settings(self, apply: bool=False):
        ...
        if not apply:
            with open(base_dir + "settings.json", "w") as _fsw:
    ...
    def set_default_settings(self):
        _fr = open(base_dir + "api/default_settings.json", "r")
        _fw = open(base_dir + "settings.json", "w")
